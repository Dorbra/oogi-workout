...
export function PreviewScreen({ state, dispatch }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 pt-5 pb-3 border-b border-white/5 flex-shrink-0">
        <button
          onClick={() => dispatch({ type: 'GO_HOME' })}
          className="glass text-zinc-400 hover:text-white text-sm font-bold px-3 py-2 rounded-xl active:scale-95 transition-all"
        >
          {t.back}
        </button>
        <div className="flex-1 text-center">
          <h2 className="font-display font-black text-white text-xl tracking-wide">{getDurationLabel(state.selectedDuration, state.selectedVariation, t)}</h2>
          <p className="text-zinc-600 text-xs mt-0.5">{items.length} {t.exercises}</p>
        </div>
        <div className="w-16" />
      </div>
        ...
        {items.map((item, idx) => {
            ...
            return (
              <div key={idx} className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(168,85,247,0.2)', background: 'rgba(168,85,247,0.04)' }}>
                ...
                {[{ ex: item.ex, exData: item.exData }, { ex: item.exB, exData: item.exDataB }].map(({ ex, exData }, j) => (
                  <div key={j} className={`px-4 py-3 flex items-center gap-3 ${j === 0 ? 'border-b border-white/5' : ''}`}>
                    <div className="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0" style={{ background: 'rgba(255,255,255,0.03)' }}>
                      <ExerciseSvg svgKey={exData.svg} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-bold text-sm leading-tight">{isHe ? exData.nameHe : exData.nameEn}</p>
                      {isHe && <p className="text-zinc-600 text-xs mt-0.5">{exData.nameEn}</p>}
                    </div>
          ...
          return (
            <div key={idx} className="glass rounded-xl px-3 py-3 flex items-center gap-3" style={{ borderLeft: '2px solid rgba(249,115,22,0.25)' }}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white font-bold text-sm leading-tight">{isHe ? item.exData.nameHe : item.exData.nameEn}</p>
                {isHe && <p className="text-zinc-600 text-xs mt-0.5">{item.exData.nameEn}</p>}
                <p className="text-zinc-700 text-xs mt-0.5">{t.rest}: {item.ex.rest}{t.sec}</p>
              </div>
              <div className="text-right flex-shrink-0">
      ...
      {/* Bottom bar */}
      <div className="px-4 pb-6 pt-3 flex-shrink-0 border-t border-white/5 space-y-3">
        <label className="flex items-center justify-between cursor-pointer">
          <span className="text-zinc-300 font-bold text-sm">{t.skipWarmup}</span>
          <div
            onClick={() => dispatch({ type: 'TOGGLE_SKIP_WARMUP' })}
            className={`w-11 h-6 rounded-full relative transition-colors ${state.skipWarmup ? 'bg-orange-500' : 'bg-zinc-800'}`}
          >
